...
//
// This file is derived from trie/trie.go (go-ethereum v1.13.15). The
// triedb/database dependency is replaced by the local database interface and
// core/types.EmptyRootHash by the local EmptyRootHash.
...
func New(id *ID, db database) (*Trie, error) {
	...
	if id.Root != (common.Hash{}) && id.Root != EmptyRootHash {
...
func NewEmpty(db database) *Trie {
	tr, _ := New(TrieID(EmptyRootHash), db)
	return tr
}
...
func (t *Trie) MustNodeIterator(start []byte) NodeIterator {
	...
	if err != nil {
		logger.Error("Unhandled trie error in Trie.NodeIterator", "err", err)
	}
	return it
}
...
func (t *Trie) MustGetNode(path []byte) ([]byte, int) {
	...
	if err != nil {
		logger.Error("Unhandled trie error in Trie.GetNode", "err", err)
	}
...
func (t *Trie) Commit(collectLeaf bool) (common.Hash, *trienode.NodeSet, error) {
	...
	if t.root == nil {
		...
		if len(paths) == 0 {
			return EmptyRootHash, nil, nil // case (a)
		}
		nodes := trienode.NewNodeSet(t.owner)
		for _, path := range paths {
			nodes.AddNode([]byte(path), trienode.NewDeleted())
		}
		return EmptyRootHash, nodes, nil // case (b)
	}
...
func (t *Trie) hashRoot() (node, node) {
	if t.root == nil {
		return hashNode(EmptyRootHash.Bytes()), nil
	}
