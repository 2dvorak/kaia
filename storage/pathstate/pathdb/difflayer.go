...
//
// This file is derived from triedb/pathdb/difflayer.go (go-ethereum v1.13.15).
...
func newDiffLayer(parent layer, root common.Hash, id uint64, block uint64, nodes map[common.Hash]map[string]*trienode.Node, states *triestate.Set) *diffLayer {
	...
	logger.Debug("Created new diff layer", "id", id, "block", block, "nodes", count, "size", common.StorageSize(dl.memory))
	return dl
}
	...
	if ok {
		...
		if ok {
			...
			if n.Hash != hash {
				...
				logger.Error("Unexpected trie node in diff layer", "owner", owner, "path", path, "expect", hash, "got", n.Hash)
