...
//
// This file is derived from triedb/pathdb/journal.go (go-ethereum v1.13.15).
...
func (db *Database) loadJournal(diskRoot common.Hash) (layer, error) {
	...
	logger.Debug("Loaded layer journal", "diskroot", diskRoot, "diffhead", head.rootHash())
	return head, nil
}
...
func (db *Database) loadLayers() layer {
	...
	if !(root == emptyRootHash && errors.Is(err, errMissJournal)) {
		logger.Info("Failed to load journal, discard it", "err", err)
	}
...
func (dl *diskLayer) journal(w io.Writer) error {
	...
	logger.Debug("Journaled pathdb disk layer", "root", dl.root, "nodes", len(dl.buffer.nodes))
	return nil
}
...
func (dl *diffLayer) journal(w io.Writer) error {
	...
	logger.Debug("Journaled pathdb diff layer", "root", dl.root, "parent", dl.parent.rootHash(), "id", dl.stateID(), "block", dl.block, "nodes", len(dl.nodes))
	return nil
}
...
func (db *Database) Journal(root common.Hash) error {
	...
	if l, ok := l.(*diffLayer); ok {
		logger.Info("Persisting dirty state to disk", "head", l.block, "root", root, "layers", l.id-disk.id+disk.buffer.layers)
	} else { // disk layer only on noop runs (likely) or deep reorgs (unlikely)
		logger.Info("Persisting dirty state to disk", "root", root, "layers", disk.buffer.layers)
	}
	start := time.Now()
	...
	db.readOnly = true
	logger.Info("Persisted dirty state to disk", "size", common.StorageSize(journal.Len()), "elapsed", common.PrettyDuration(time.Since(start)))
	return nil
}
