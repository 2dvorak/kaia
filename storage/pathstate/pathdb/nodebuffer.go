...
//
// This file is derived from triedb/pathdb/nodebuffer.go (go-ethereum v1.13.15).
// The ethdb key-value store types are replaced by Kaia's storage/database
// types, and the revert path (used only for state-history rollback, which is
// disabled here) has been removed.
...
func (b *nodebuffer) node(owner common.Hash, path []byte, hash common.Hash) (*trienode.Node, error) {
	...
	if n.Hash != hash {
		...
		logger.Error("Unexpected trie node in node buffer", "owner", owner, "path", path, "expect", hash, "got", n.Hash)
...
func (b *nodebuffer) updateSize(delta int64) {
	...
	s := b.size
	b.size = 0
	logger.Error("Invalid pathdb buffer size", "prev", common.StorageSize(s), "delta", common.StorageSize(delta))
}
...
func (b *nodebuffer) setSize(size int, db database.Database, clean *fastcache.Cache, id uint64) error {
	b.limit = uint64(size)
...
func (b *nodebuffer) flush(db database.Database, clean *fastcache.Cache, id uint64, force bool) error {
	...
	logger.Debug("Persisted pathdb nodes", "nodes", len(b.nodes), "bytes", common.StorageSize(size), "elapsed", common.PrettyDuration(time.Since(start)))
	b.reset()
	return nil
}
...
// into clean cache.
func writeNodes(batch database.Batch, nodes map[common.Hash]map[string]*trienode.Node, clean *fastcache.Cache) (total int) {
	for owner, subset := range nodes {
		for path, n := range subset {
			if n.IsDeleted() {
