...
//
// This file is derived from triedb/pathdb/disklayer.go (go-ethereum v1.13.15).
// State-history writing/pruning in commit() and the revert() path have been
// removed since state history is disabled in this port. The keccak hasher is
// adapted to Kaia's crypto/sha3.
...
func (dl *diskLayer) Node(owner common.Hash, path []byte, hash common.Hash) ([]byte, error) {
	...
	if dl.cleans != nil {
		if blob := dl.cleans.Get(nil, key); len(blob) > 0 {
			...
			logger.Error("Unexpected trie node in clean cache", "owner", owner, "path", path, "expect", hash, "got", got)
		}
	...
	if nHash != hash {
		...
		logger.Error("Unexpected trie node in disk", "owner", owner, "path", path, "expect", hash, "got", nHash)
...
// keccakState wraps sha3.state, adding a Read method to pull a variable amount
// of data from the hash state.
