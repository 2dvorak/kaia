...
func initGenesis(ctx *cli.Context) error {
	...
	usePathTrieArchive := ctx.Bool(utils.PathTrieArchiveFlag.Name)
	if usePathTrieArchive {
		usePathTrie = true
	}
	...
	for _, name := range []string{"chaindata"} { // Removed "lightchaindata" since Kaia doesn't use it
		dbc := &database.DBConfig{
			...
			UseFlatTrie: useFlatTrie, UsePathTrie: usePathTrie, UsePathTrieArchive: usePathTrieArchive,
		}
