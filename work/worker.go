...
type worker struct {
	...
	// update() drives finalizeCh/pendingWork transitions, while the existing
	// locks still guard lifecycle edges and snapshot access.
...
func (self *worker) commitNewWork(trigger newWorkTrigger) {
	...
	// V1 uses PendingSnapshot (no error). Same metric names as V0 for direct comparison.
...
func (self *worker) handleFinalizedBlock(result *consensus.ExecutionResult) {
	...
	// hot-path latency while still preserving per-module error logs.
...
func (env *Task) ApplyTransactions(txs *types.TransactionsByPriceAndNonce, bc BlockChain, nodeAddr common.Address, txBundlingModules []builder.TxBundlingModule) []*types.Log {
	...
	// (V1 keeps the heap intact and calls ArrayifyByCount lazily below; the
	// minerArrayifyTimer is updated around that lazy call instead of here.)
	totalTxs := txs.Len()
	...
	// Read the mutable CLI-tuned limit once for this block-building round.
