...
// ContentFrom retrieves the pending and queued transactions of a specific address.
...
func (pool *TxPool) addTx(tx *types.Transaction, local bool) error {
	// Single-tx submissions are on the RPC latency path. Cache the sender
	// synchronously before taking pool.mu instead of blocking on the shared
	// async sender-cacher queue and then potentially validating before the
